...
export class DependencyScanner {
  ...
  public static resolveModuleName(name: string, localModules: Set<string>, availableSharedModules: Map<string, SharedModuleInfo>): string | null {
  ...
  public static detectReferencedModules(srcDir: string, availableSharedModules: Map<string, SharedModuleInfo>): Set<string> {
    ...
    if (sourceFilePaths.size === 0) {
      ...
      if (fs.existsSync(data7ModulesDir)) {
        ...
        existingFiles.forEach(file => {
          try { fs.unlinkSync(path.join(data7ModulesDir, file)); } catch {}
        });
      }
      return synced;
    }
