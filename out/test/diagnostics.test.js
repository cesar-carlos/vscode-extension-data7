...
const mockTextDocuments = vscode.workspace.textDocuments;
(0, node_test_1.test)('DiagnosticsLinter - Advanced Validation rules', () => {
...
End Namespace
  `;
    ...
    mockTextDocuments.push({ uri: { toString: () => dummyFile1Uri, fsPath: 'dummy/mod_resources.bas' } }, { uri: { toString: () => dummyPrincipalUri, fsPath: 'dummy/Principal.bas' } }, { uri: { toString: () => testFileUri, fsPath: 'dummy/test_file.bas' } });
    const runDiagnostics = (content) => {
        ...
        const mockDocument = {
            ...
            getText: () => content,
            split: (regex) => content.split(regex),
            lineCount: content.split(/\r?\n/).length,
            lineAt: (lineIdx) => ({
                text: content.split(/\r?\n/)[lineIdx]
            })
        };
        const idx = mockTextDocuments.findIndex((doc) => doc.uri.toString() === testFileUri);
        if (idx !== -1) {
            mockTextDocuments[idx] = mockDocument;
        }
        return diagnostics_1.DiagnosticsLinter.runAdvancedDiagnostics(mockDocument, indexer);
    };
...
(0, node_test_1.test)('WorkspaceSymbolIndexer - validateCache', () => {
    const indexer = symbol_indexer_1.WorkspaceSymbolIndexer.getInstance();
...
(0, node_test_1.test)('WorkspaceSymbolIndexer - delete and rename folders', () => {
    const indexer = symbol_indexer_1.WorkspaceSymbolIndexer.getInstance();
    // Set up mock paths
    ...
    mockTextDocuments.length = 0;
    mockTextDocuments.push({ uri: { toString: () => file1Uri, fsPath: file1 } }, { uri: { toString: () => file2Uri, fsPath: file2 } });
    ...
    mockTextDocuments.length = 0;
    mockTextDocuments.push({ uri: { toString: () => newFile1Uri, fsPath: 'c:\\dummy_project\\src\\mod_card\\form\\card.bas' } }, { uri: { toString: () => newFile2Uri, fsPath: 'c:\\dummy_project\\src\\mod_card\\helper.bas' } });
...
(0, node_test_1.test)('WorkspaceSymbolIndexer - dynamic cache validation on lookup', () => {
    const indexer = symbol_indexer_1.WorkspaceSymbolIndexer.getInstance();
...
(0, node_test_1.test)('SystemLibrary - inheritance chain resolution (StringList -> TStringList -> TStrings -> TPersistent -> TObject)', () => {
    const { SYSTEM_SYMBOLS } = require('../system-library');
    // Helper: find a symbol by name (class-level) and optional containerName
    const findClass = (name, container) => SYSTEM_SYMBOLS.find((s) => s.name === name && s.kind === 'class' && (container === undefined || s.containerName === container));
    // Helper: find member of a class
    const findMember = (memberName, container) => SYSTEM_SYMBOLS.find((s) => s.name === memberName && s.containerName === container);
    ...
    const removedTStrings = SYSTEM_SYMBOLS.find((s) => s.name === 'TStrings' && s.containerName === 'System.Classes');
    ...
    const removedTStringList = SYSTEM_SYMBOLS.find((s) => s.name === 'TStringList' && s.containerName === 'System.Classes');
    ...
    const globalTObject = SYSTEM_SYMBOLS.find((s) => s.name === 'TObject' && s.kind === 'class' && !s.containerName);
    ...
    const globalTPersistent = SYSTEM_SYMBOLS.find((s) => s.name === 'TPersistent' && s.kind === 'class' && !s.containerName);
