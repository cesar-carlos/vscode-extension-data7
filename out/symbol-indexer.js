...
exports.WorkspaceSymbolIndexer = exports.SymbolParser = void 0;
...
class SymbolParser {
    ...
    static parseBasFile(fileUri, content) {
...
class WorkspaceSymbolIndexer {
    constructor() {
        this.cache = new Map(); // fileUri -> FileSymbols
    }
    static getInstance() {
     ...
     */
    async indexWorkspace(workspaceFolders) {
        this.validateCache();
        if (!workspaceFolders)
            return;
    ...
    deleteWorkspaceFolder(deletedPath) {
     ...
     */
    updateFileContent(fileUri, content) {
        try {
            const parsed = SymbolParser.parseBasFile(fileUri, content);
     ...
     */
    findSymbolByName(name, contextFileUri) {
        const lowerName = name.toLowerCase();
    ...
    findClassMember(className, memberName) {
        ...
        let match = allSymbols.find(s => s.containerName?.toLowerCase() === className.toLowerCase() &&
        ...
        while (currentClass && currentClass.inheritsFrom && !visited.has(currentClass.name.toLowerCase())) {
            ...
            match = allSymbols.find(s => s.containerName?.toLowerCase() === parentName.toLowerCase() &&
                s.name.toLowerCase() === memberLower);
            if (match) {
                if (this.isFileValid(match.fileUri)) {
                    return match;
