...
exports.D7BasicCompletionProvider = exports.TypeResolver = void 0;
...
class TypeResolver {
    static getVariableType(varName, document, position, indexer) {
        ...
        for (let i = position.line; i >= 0; i--) {
            ...
            if (lineText.startsWith("'") || lineText.toLowerCase().startsWith('rem '))
                continue;
        ...
        if (fileSyms) {
            ...
            const currentMethod = fileSyms.symbols.find(s => s.kind === 'method' &&
            ...
            const currentClass = fileSyms.symbols.find(s => s.kind === 'class' &&
                ...
                (s.range.endLine === undefined || position.line <= s.range.endLine));
            if (currentClass) {
                const field = fileSyms.symbols.find(s => s.containerName?.toLowerCase() === currentClass.name.toLowerCase() &&
            ...
            const globalVar = fileSyms.symbols.find(s => s.name.toLowerCase() === varLower &&
                s.kind === 'variable' &&
                !s.containerName);
            if (globalVar) {
     ...
     */
    static findClassSymbol(qualifiedOrSimpleName, indexer) {
        ...
        if (qualifiedOrSimpleName.includes('.')) {
            ...
            const exact = system_library_1.SYSTEM_SYMBOLS.find(s => s.kind === 'class' &&
                s.name.toLowerCase() === namePart &&
                s.containerName?.toLowerCase() === nsPart);
            if (exact)
                return exact;
            // 2. Name-only fallback in system library
            const byName = system_library_1.SYSTEM_SYMBOLS.find(s => s.kind === 'class' && s.name.toLowerCase() === namePart);
            if (byName)
                return byName;
        ...
        return (system_library_1.SYSTEM_SYMBOLS.find(s => s.kind === 'class' && s.name.toLowerCase() === lower) ??
            indexer.findSymbolByName(qualifiedOrSimpleName));
    }
    static getInheritedMembers(className, indexer) {
        const members = [];
        const visited = new Set();
        const resolveMembers = (currentClassName) => {
            ...
            const systemMembers = system_library_1.SYSTEM_SYMBOLS.filter(s => s.containerName !== undefined &&
                (s.containerName.toLowerCase() === key ||
                    s.containerName.toLowerCase() === shortName));
            members.push(...systemMembers);
            const workspaceMembers = indexer.getAllSymbols().filter(s => s.containerName !== undefined &&
                (s.containerName.toLowerCase() === key ||
                    s.containerName.toLowerCase() === shortName));
...
class D7BasicCompletionProvider {
    constructor() {
     ...
     */
    static getAllMembersForType(typeName, indexer) {
        const members = [];
        const visited = new Set();
        const collect = (currentTypeName) => {
    ...
    provideCompletionItems(document, position, token, context) {
        ...
        if (dotIndex !== -1 && textBeforeCursor.substring(dotIndex + 1).trim() === textBeforeCursor.substring(dotIndex + 1)) {
            ...
            if (lastWordMatch) {
                ...
                const isNamespaceOrStaticClass = system_library_1.SYSTEM_SYMBOLS.some(s => (s.name.toLowerCase() === triggerLower && (s.kind === 'namespace' || s.kind === 'class')) ||
                    (s.containerName?.toLowerCase() === triggerLower)) || this.indexer.getAllSymbols().some(s => (s.name.toLowerCase() === triggerLower && (s.kind === 'namespace' || s.kind === 'class')) ||
                    (s.containerName?.toLowerCase() === triggerLower));
                if (isNamespaceOrStaticClass) {
                    // Suggest members belonging to this namespace or class
                    const systemMembers = system_library_1.SYSTEM_SYMBOLS.filter(s => s.containerName?.toLowerCase() === triggerLower);
        ...
        system_library_1.SYSTEM_SYMBOLS.forEach(s => {
    ...
    createCompletionItem(s, document) {
        ...
        if (s.containerName && (s.kind === 'class' || s.kind === 'structure' || s.kind === 'delegate')) {
            ...
            if (!isVclOrSystem) {
                ...
                if (!isCurrentNamespace && !isAlreadyImported) {
                    ...
                    const importEdit = new vscode.TextEdit(new vscode.Range(new vscode.Position(insertLine, 0), new vscode.Position(insertLine, 0)), `Imports ${s.containerName}\r\n`);
...
exports.D7BasicCompletionProvider = D7BasicCompletionProvider;
